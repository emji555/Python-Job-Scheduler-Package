...
from package_name.core.executor import execute_envelope
from package_name.core.result import JobHandle, JobStatus
from package_name.exceptions import ConfigurationError, UnsupportedCapabilityError
    ...
    def _ensure_celery(self) -> Any:
        ...
        try:
            ...
            raise ConfigurationError(
                "CeleryBackend requires the 'celery' extra: pip install 'package-name[celery]'"
            ) from exc
        if not self._broker_url:
            raise ConfigurationError("CeleryBackend requires broker_url / PACKAGE_BROKER_URL")
        self._celery = Celery("package_name", broker=self._broker_url, backend=self._result_backend)

        backend_self = self

        @self._celery.task(name="package_name.execute_envelope", bind=True)  # type: ignore[misc]
