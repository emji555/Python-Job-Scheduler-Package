...
from workloom import __version__
from workloom.app import configure, get_current_app
...
@click.group()
@click.version_option(__version__, prog_name="workloom")
def main() -> None:
    """Workloom — jobs, queues, scheduling, and monitoring."""
...
def doctor_cmd() -> None:
    ...
    if app.settings.lock_backend == "redis" or app.settings.backend == "celery":
        ...
        if not url:
            checks.append(("broker_url", False, "WORKLOOM_BROKER_URL not set"))
        else:
            try:
                import redis
