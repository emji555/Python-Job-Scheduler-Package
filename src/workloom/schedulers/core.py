...
from workloom.exceptions import SchedulerError
from workloom.locks.memory import new_lock_owner
...
class ScheduledJob:
    ...
    def __init__(self, schedule: Schedule, job: Job[Any, Any] | Callable[..., Any]) -> None:
        ...
        if isinstance(job, Job):
            ...
            from workloom.core.registry import qualify_name
...
def try_dispatch_entry(app: App, entry: ScheduleEntry, *, now: datetime | None = None) -> bool:
    ...
    try:
        if entry.on_one_server and not acquire(
            f"workloom:schedule:one:{entry.id}", ttl=max(entry.overlap_ttl, 60)
        ):
            return False
        if entry.without_overlapping and not acquire(
            f"workloom:schedule:overlap:{entry.id}", ttl=entry.overlap_ttl
        ):
            return False

        # Resolve job and dispatch
        definition = app.registry.get(entry.job_name)
        from workloom.core.jobs import Job as JobWrapper
        ...
        for key, own in locks_held:
            if key.startswith("workloom:schedule:one:"):
